...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        ...
                        f"Series Description: 'HippoCropPred'\n"
                        f"Date: {header.StudyDate}\n"
                        f"Anterior: {inference['anterior']}\n"
                        f"Posterior: {inference['posterior']}\n"
...
def get_series_for_inference(path):
    ...
    for d in dicoms:
        if (0x0008, 0x103E) in d:
            if d.SeriesDescription == 'HippoCrop':
...
if __name__ == "__main__":
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path='unet/model.pth')
